...
const (
	...
	// ParentEnvTag is the root tag for stage environment variables when the task is entrusted
...
func newCapCommFromEnvironment(ctx context.Context, configFile string) *CapComm {
...
// False means do no process futher
func (capComm *CapComm) isFiltered(filter *Filter) bool {
	if filter == nil {
		return false
	}
